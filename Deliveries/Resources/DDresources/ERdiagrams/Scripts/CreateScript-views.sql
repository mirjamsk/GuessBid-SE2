...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `guessbid_db`.`active_auctions` (`auction_id` INT, `seller_id` INT, `name` INT, `description` INT, `end_time` INT, `winning_bid_id` INT);

-- -----------------------------------------------------
-- Placeholder table for view `guessbid_db`.`currently_winning_bid`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `guessbid_db`.`currently_winning_bid` (`bid_id` INT, `bidder_id` INT, `bid_auction_id` INT, `amount` INT);

-- -----------------------------------------------------
-- Placeholder table for view `guessbid_db`.`finished_auctions`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `guessbid_db`.`finished_auctions` (`auction_id` INT, `seller_id` INT, `name` INT, `description` INT, `end_time` INT, `winning_bid_id` INT);
...
USE `guessbid_db`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `guessbid_db`.`active_auctions` AS select `guessbid_db`.`auction`.`auction_id` AS `auction_id`,`guessbid_db`.`auction`.`seller_id` AS `seller_id`,`guessbid_db`.`auction`.`name` AS `name`,`guessbid_db`.`auction`.`description` AS `description`,`guessbid_db`.`auction`.`end_time` AS `end_time`,`guessbid_db`.`auction`.`winning_bid_id` AS `winning_bid_id` from `guessbid_db`.`auction` where (`guessbid_db`.`auction`.`end_time` > now());

-- -----------------------------------------------------
-- View `guessbid_db`.`currently_winning_bid`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `guessbid_db`.`currently_winning_bid` ;
DROP TABLE IF EXISTS `guessbid_db`.`currently_winning_bid`;
USE `guessbid_db`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `guessbid_db`.`currently_winning_bid` AS select `b1`.`bid_id` AS `bid_id`,`b1`.`bidder_id` AS `bidder_id`,`b1`.`bid_auction_id` AS `bid_auction_id`,`b1`.`amount` AS `amount` from `guessbid_db`.`bid` `b1` where ((`b1`.`bid_auction_id`,`b1`.`amount`) = (select `b2`.`bid_auction_id`,`b2`.`amount` from `guessbid_db`.`bid` `b2` where (`b1`.`bid_auction_id` = `b2`.`bid_auction_id`) group by `b2`.`bid_auction_id`,`b2`.`amount` having (count(0) = 1) order by `b2`.`amount` limit 1));
